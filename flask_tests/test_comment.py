...
def test_get_comments(client, auth_headers):
    ...
    """
    mock_comment = {"id": "1", "content": "Test Comment", "author": "test_user"}
    with patch(
        "api.models.comment.Comment.get_by_post", return_value=[mock_comment]
    ):
...
def test_create_comment_success():
    ...
    """
    with patch("api.utils.firestore_helpers.get_db") as mock_get_db:
        mock_db = MagicMock()
        ...
        mock_comment_ref.id = "mock_comment_id"
        mock_post.reference.collection.return_value.document.return_value = (
            mock_comment_ref
        )
        ...
        assert comment_id == "mock_comment_id"


def test_create_comment_post_not_found():
    """
    Test that NotFound is raised when post is not found.
    """
    with patch("api.utils.firestore_helpers.get_db") as mock_get_db:
        mock_db = MagicMock()
        mock_get_db.return_value = mock_db
        ...
        with pytest.raises(NotFound, match="404 Post not found"):
            Comment.create(
                "mock_post_id", {"email": "testuser@usc.edu"}, "Test Comment"
            )
...
def test_create_comment_firestore_error():
    ...
    with patch("api.utils.firestore_helpers.get_db") as mock_get_db:
        ...
        mock_post.reference.collection.return_value.document.side_effect = GoogleAPICallError(
            "Firestore error"
        )

        with pytest.raises(GoogleAPICallError, match="Firestore error"):
            Comment.create(
                "mock_post_id", {"email": "testuser@usc.edu"}, "Test Comment"
            )
