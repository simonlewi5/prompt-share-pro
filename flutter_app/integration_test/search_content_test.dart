...
void main() {
  ...
  group('Search by Content Flow Test', () {
    testWidgets('Change the search type to Full Text', (tester) async {
      ...
      // Open the filter menu and select "Full Text"
      final filterMenu = find.byKey(const Key('popupMenuButton'));
      await tester.tap(filterMenu);
      await tester.pumpAndSettle();

      final llmFilterOption = find.text('Full Text');
      await tester.tap(llmFilterOption);
      await tester.pumpAndSettle();

      // Verify that the filter has been set to "Full Text"
      expect(find.textContaining('Search by Full Text'), findsOneWidget);
    });
  });
}