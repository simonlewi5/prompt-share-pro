...
void main() {
  ...
  group('Search by Title Flow Test', () {
    testWidgets('Change the search type to Title', (tester) async {
      ...
      // Open the filter menu and select "Title"
      final filterMenu = find.byKey(const Key('popupMenuButton'));
      await tester.tap(filterMenu);
      await tester.pumpAndSettle();

      final llmFilterOption = find.text('Title');
      await tester.tap(llmFilterOption);
      await tester.pumpAndSettle();

      // Verify that the filter has been set to "Title"
      expect(find.textContaining('Search by Title'), findsOneWidget);
    });
  });
}